...
def display_instances(image, boxes, masks, obj_names, colors,
    ...
    for i in range(N):
        ...
        if apply_box:
            ...
            if scores is not None:
                text_label  = f"{label} : {round(score, 2)}"

            ax.text(x1, y1, text_label,
    ...
    plt.savefig(os.path.join(save_pred_dir, image_file_name),bbox_inches='tight', pad_inches=-0.5,orientation= 'landscape')
    plt.close()
    print("Image saved.")